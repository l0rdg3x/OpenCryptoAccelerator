...
"""Run the UART console integration tests under the project-local Verilator.
...
RTL = ROOT / "oca" / "hw" / "rtl"
SOURCES = [RTL / "oca_uart_rx.sv", RTL / "oca_uart_tx8.sv",
           RTL / "oca_fifo.sv", RTL / "oca_console.sv",
           RTL / "oca_uart_console.sv"]
...
def main() -> int:
    ...
    runner.build(
        sources=SOURCES,
        hdl_toplevel="oca_uart_console",
        build_dir=SIM_DIR / "sim_build_uart_console",
        always=True,
    )
    (SIM_DIR / "results.xml").unlink(missing_ok=True)  # never grade a stale file
    runner.test(
        hdl_toplevel="oca_uart_console",
        test_module="test_uart_console",
        test_dir=SIM_DIR,
        build_dir=SIM_DIR / "sim_build_uart_console",
    )
