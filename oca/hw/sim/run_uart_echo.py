...
"""Run the UART echo tests under the project-local Verilator.
...
def main() -> int:
    ...
    runner.build(
        sources=SOURCES,
        hdl_toplevel="oca_uart_echo",
        build_dir=SIM_DIR / "sim_build_uart_echo",
        always=True,
    )
    (SIM_DIR / "results.xml").unlink(missing_ok=True)  # never grade a stale file
    runner.test(
        hdl_toplevel="oca_uart_echo",
        test_module="test_uart_echo",
        test_dir=SIM_DIR,
        build_dir=SIM_DIR / "sim_build_uart_echo",
    )
