...
"""Run the SLIP encoder tests under the project-local Verilator.
...
def main() -> int:
    ...
    runner.build(
        sources=SOURCES,
        hdl_toplevel="oca_slip_tx",
        build_dir=SIM_DIR / "sim_build_slip_tx",
        always=True,
    )
    (SIM_DIR / "results.xml").unlink(missing_ok=True)  # never grade a stale file
    runner.test(
        hdl_toplevel="oca_slip_tx",
        test_module="test_slip_tx",
        test_dir=SIM_DIR,
        build_dir=SIM_DIR / "sim_build_slip_tx",
    )
